...
import { RegisterCompany } from 'src/students/application/commands/register-company.command';
...
export class RegisterCompanyHandler
  ...
  async execute(command: RegisterCompany) {
    ...
    studentId = Number(companyTypeORM.id);
    company.changeId(StudentId.of(studentId));
